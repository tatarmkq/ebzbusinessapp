...
// MTA - end			
	...
	change_courseorder(refresher: any, order: any): void
	{
		...
		Promise.all(promises).finally(() => {
			...
			this.fetchCourses().finally(() => {
				...
				Console.log("SORTORDER-REFRESH : SUCCESSFULL");
    ...
    protected fetchCourses(): Promise<any> {
		...
		else if (this.courseorder == 1)
		{
			return this.coursesProvider.getUserCoursesSortedFna().then((courses) => {
				// MTA - test :
				console.log("KURSE 2 = " + JSON.stringify(courses));
				const promises = [],
					courseIds = courses.map((course) => {
					return course.id;
				});
		...
		else if (this.courseorder == 2)
		{
			return this.coursesProvider.getUserCoursesSortedFnd().then((courses) => {
				// MTA - test :
				console.log("KURSE 2 = " + JSON.stringify(courses));
				const promises = [],
					courseIds = courses.map((course) => {
		...
		else if (this.courseorder == 3)
		{
			return this.coursesProvider.getUserCoursesSortedSdd().then((courses) => {
				// MTA - test :
				console.log("KURSE 2 = " + JSON.stringify(courses));
				const promises = [],
					courseIds = courses.map((course) => {
		...
		else if (this.courseorder == 4)
		{
			return this.coursesProvider.getUserCoursesSortedSda().then((courses) => {
				// MTA - test :
				console.log("KURSE 2 = " + JSON.stringify(courses));
				const promises = [],
					courseIds = courses.map((course) => {
...
// MTA - start : Sorting of courses on course page	
	sort_courses() : void
	{
