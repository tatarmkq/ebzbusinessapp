...
export class CoreWSProvider {
    ...
    call(method: string, data: any, preSets: CoreWSPreSets): Promise<any> {
		if((method == "core_get_ebz_homsite_courses") || (method == "gradereport_user_get_grades_table"))
			console.log("CHEPOINT_5a_TEST");
		// MTA - marker 10 (WS-available) 
		...
		if((method == "core_get_ebz_homsite_courses") || (method == "gradereport_user_get_grades_table"))
		{	
			console.log("CHEPOINT_5c_10");
			console.log("preSets.siteUrl_forLetPromise = " +  preSets.siteUrl);
			console.log("data_forLetPromise = " + JSON.stringify(data));
		}	
		
        let promise = this.getPromiseHttp('post', preSets.siteUrl, data);
		
		if((method == "core_get_ebz_homsite_courses") || (method == "gradereport_user_get_grades_table"))
			console.log("CHEPOINT_5c_10_1");
		
        if (!promise) {
            // There are some ongoing retry calls, wait for timeout.
            if (this.retryCalls.length > 0) {
				if((method == "core_get_ebz_homsite_courses") || (method == "gradereport_user_get_grades_table"))
					console.log("CHEPOINT_5c_20");
				...
				console.log("CHEPOINT_5c_30");
                promise = this.performPost(method, siteUrl, data, preSets);
            }
        }
		if((method == "core_get_ebz_homsite_courses") || (method == "gradereport_user_get_grades_table"))
		{	
			console.log("CHEPOINT_5c_40");
    ...
    protected getPromiseHttp(method: string, url: string, params?: any): any {
		//if((method == "core_get_ebz_homsite_courses") || (method == "gradereport_user_get_grades_table"))
		if(params != 'undefined')	
			console.log("LET_PROMISE_params = " + JSON.stringify(params));		
        ...
        if (typeof this.ongoingCalls[queueItemId] != 'undefined') {
			...
			{	
				...
				//if(str.includes("core_get_ebz_homsite_courses") || str.includes("gradereport_user_get_grades_table"))
				//	console.log("FEHLERFREIER_FALL_1");
			}
			console.log("BERECHNETER_PROMISE = " + this.ongoingCalls[queueItemId]);
    ...
    performPost(method: string, siteUrl: string, ajaxData: any, preSets: CoreWSPreSets): Promise<any> {
        ...
        promise = promise.then((data: any) => {
            ...
            if (!data && !preSets.responseExpected) {
				console.log("LEERES_OBJEKT!");
                data = {};
            }

            if (!data) {
				console.log("LEERES_OBJEKT --> SERVER_CONN_ERR!");
                return Promise.reject(this.createFakeWSError('core.serverconnection', true));
            } else if (typeof data != preSets.typeExpected) {
				console.log("FALSCHER_RÜCKGABEDATENTYP!");
